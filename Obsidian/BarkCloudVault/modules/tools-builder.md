...
- **Порты сервисов не публикуются:** все `cloud-*` доступны только внутри
  `barkcloud-network`; наружу порты публикует reverse-proxy nginx. При выключенном nginx
...
- **Nginx (при включённом тумблере):** в выходную папку пишутся `nginx/cloud.barkfluff.conf`
  ...
  совпадающий с `Backend/nginx/cloud.barkfluff.conf`; web-vhost содержит отдельный небуферизуемый
  `/file-upload/` route прямо в HTTP/1 Files для data plane [[modules/upload-2]].
- **Генерация — сборка секций строк** (`BackendComposeGenerator`), не парсинг YAML.
  ...
  соответствует `Backend/docker-compose.yml`; режим со встроенным nginx остаётся доступен
  как вариант reverse-proxy.
