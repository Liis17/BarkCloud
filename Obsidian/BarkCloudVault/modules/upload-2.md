...
- Files использует `AWSSDK.S3 4.0.103.3` / `AWSSDK.Core 4.0.102.6`: более старая `AWSSDK.S3 4.0.23.5` оборачивала Kestrel `Request.Body` в требующий seek `PartialWrapperStream` и падала до отправки R2-части. Регрессионный тест проходит через настоящий AWS pipeline с forward-only stream и подменённым HTTP transport, не буферизуя часть целиком в приложении.
