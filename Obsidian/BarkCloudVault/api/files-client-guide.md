...
Для полного счётчика и размера галереи, не зависящих от уже загруженных страниц, вызвать `CloudApi.GetUserMediaStats({ kind })`. Ответ содержит `total_count` и `total_size_bytes`; он использует те же правила видимости, что и галерея (готовые оригиналы, без превью и файлов в корзине).
