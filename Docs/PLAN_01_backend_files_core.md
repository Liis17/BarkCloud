...
**Подзадача 1.5b (бэкафилл — опционально, по умолчанию включаем):** разовая очистка уже накопленных осиротевших `AlbumItems` (от файлов, удалённых до фикса) — по образцу `LegacyPreviewBackfillService`. Иначе `items_count` остаётся раздутым и возможны мёртвые обложки. Если решим не делать — явно отметить, что чинится только дальнейшая утечка.
