...
- Публичные ссылки (CloudApi): `GET shares` (`ListMyShares`), `POST shares` `{fileId, name}` (`CreateShare`), `POST shares/revoke` `{shareId}` (`RevokeShare`, POST т.к. в `api.ts` нет `apiDelete`). Дружелюбный `url = {scheme}://{host}/s/{token}` собирается в `CloudApiEndpoints.ShareJson` из `http.Request` (Web — владелец публичного роута, Files остаётся host-agnostic). Питает страницу `/shared`.
...
**Публичные папки** (`/api/folder-shares` GET/POST + `/revoke`): `CreateFolderShare`/`ListMyFolderShares`/`RevokeFolderShare`. `FolderShareJson` собирает `url = {origin}/f/{token}`, `kind=folder`. В «Мои публичные» вкладка `SharedPage` грузит и `/api/shares`, и `/api/folder-shares`, мержит (папки — `LinkCard` с иконкой папки; отзыв роутится по `kind`). Папку делают публичной из контекстного меню в «Файлах» (`dirMenu` → `createFolderShare`). **Публичная страница папки** `/f/:token` (`pages/PublicFolderPage.tsx`, вне `AppShell`, анонимна): `fetch GET /f/{token}/list?dir=` (`ResolveFolderShare` сервисным токеном) → грид подпапок + файлов; навигация по подпапкам, клик по фото/видео — встроенный вьювер (img/video по temp-URL), по файлу — скачивание. Fallback в `Program.cs` пускает `/f/*` без авторизации (как `/v/*`).

**Публичные альбомы** (`/api/album-shares` GET/POST + `/revoke`, на `CloudApi`): `CreateAlbumShare`/`ListMyAlbumShares`/`RevokeAlbumShare`. `AlbumShareJson` собирает `url = {origin}/al/{token}`, `kind=album`. В «Мои публичные» вкладка `SharedPage` грузит и `/api/album-shares` (иконка фото; отзыв по `kind`). Альбом делают публичным кнопкой «Поделиться» в `AlbumDetail` (`lib/share.createAlbumShare`). **Публичная страница альбома** `/al/:token` (`pages/PublicAlbumPage.tsx`, вне `AppShell`, анонимна): `fetch GET /al/{token}/list?cursor*` (`ResolveAlbumShare` сервисным токеном) → грид элементов с пагинацией «Показать ещё», встроенный вьювер. Fallback в `Program.cs` пускает `/al/*` без авторизации.
