...
- **Системные папки виртуальные** — не хранятся в БД, отдаются кодом с well-known id `sys-recent-media` / `sys-recent-docs` / `sys-large` / `sys-screenshots` / `sys-duplicate-media` / `sys-duplicate-files`. Критерии захардкожены в `Domain/SystemDynamicFolders.cs`. Нельзя редактировать/удалять.
  ...
  - «Недавние документы» — `CreatedAt` за последние **3 дня** И `MediaKind ∈ {документ, аудио, прочее}` (все не-медиа, значение правила `"3,4,0"`); вид — список.
  - «Большие файлы» — `Size > 100 МБ` (104857600 байт); вид — список.
...
- Регистронезависимость по имени и устройствам — через `ToLower()` перед применением строкового оператора (стиль `ListUserImagesPage`).
...
Дата загрузки, дата съёмки (`FileMetadata.TakenAt`), размер, имя, формат (`MediaKind`), расширение, ширина/высота изображения, устройство загрузки (`UploadFile.UploadDeviceName`), устройство из метаданных (`FileMetadata.CameraMake + CameraModel`). Операторы: за последние N дней / до / после / больше / меньше / содержит / равно / начинается / заканчивается.

`DF_DEVICE = 9` — legacy-код устройства загрузки; `DF_METADATA_DEVICE = 10` — объединённое отображаемое имя устройства из Make и Model (например, `Apple iPhone 17 Pro Max`). Фильтр метаданных не использует значение устройства загрузки как fallback.
