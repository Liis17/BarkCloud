...
- `IMultipartUploadStore.cs` / `S3MultipartUploadStore.cs` — Files-owned V2 multipart seam: initiate/upload/list/complete/abort/head; R2-части используют unsigned non-chunked payload. Metadata имени файла отправляется только для печатного ASCII, Unicode-имя остаётся в `UploadSession`; пустой/null `Parts` от S3 нормализуется в пустой список
