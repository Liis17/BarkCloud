...
public sealed class AuthGateway
{
    ...
    public AuthGateway(IdentityApi.IdentityApiClient identity, IConfiguration configuration, ILogger<AuthGateway> logger)
    {
        _identity = identity;
        _logger = logger;
        _cookieSecure = configuration.GetValue("App:CookieSecure", false);
        _appName = configuration["App:AppName"] ?? "BarkCloud Web";
        _appVersion = configuration["App:Version"] ?? "v1.0.0";
