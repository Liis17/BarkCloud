...
- **Общие компоненты** (`src/components/`): `Icon`, `MediaThumb` (srcset+sizes), `Lightbox` (оригинал через `/api/files/download`; при переданном пропе `actions` — **плавающая панель действий** `.lb-actions` под изображением: поделиться (мини-меню: публичная/временная ссылка/пользователю), добавить в альбом (меню альбомов), показать в папке, скачать оригинал, скопировать изображение в буфер (только фото: fetch → canvas → PNG → `ClipboardItem`), свойства, удалить; иконки с title-тултипами; при удалении/опустении items индекс клампится/вьювер закрывается; без `actions` — прежняя кнопка «Скачать оригинал». `actions` = `useMediaActions().api` (`MediaActionsApi`) — модалки подтверждения/свойств/шаринга рендерятся в overlay родителя, `.modal-backdrop` поднят до z-index 1150 (выше `.lightbox` 1100). Панель подключена на Фото/Видео, в `DynamicFolderDetail`, `MemoriesStrip`, результатах поиска (`MediaSearchResults`), Избранном и `AlbumDetail`; без панели остаётся только Lightbox в `FilesPage` (инспектор)), `Modal`/`EmptyState`/`Loading`/`ContextMenu`/`RenameModal`/`ConfirmModal`/`PropertiesModal`/`SelectionBar` (плавающая панель групповых действий), альбомы (`AlbumCard`/`AlbumFormModal`/`PickMediaModal`/`AlbumDetail`).
...
- Галерея (CloudApi): `GET cloud/media?kind=photo|video&limit=&cursorAt=&cursorId=` (`ListUserMedia`, cursor-пагинация), `POST cloud/media/delete { fileId }` (`DeleteUserMedia`). Карточка галереи (`CloudJson.MediaItem`) несёт `entryIds`/`entryNames`/`entriesCount` (поле `UserImageItem.entry_ids` в proto) — нужны контекстному меню для переименования/перехода в папку через `entry/rename`·`GetPath`; удаление галерейных карточек всегда идёт по `fileId` и помещает медиа в корзину, включая медиа без записи каталога.
...
Создаются ссылки из контекстного меню любого файла («Файлы», «Фото», «Видео») — `lib/share.createShare(fileId, name, toast)` шлёт `POST /api/shares` и кладёт в буфер ссылку на **страницу просмотра** `/v/{token}` (прямое скачивание — `/s/{token}`). Пункт **«Поделиться с пользователем»** (`ShareWithUserModal`) ищет получателей (`/api/shared/users/search`) и выдаёт приватный грант (`/api/shared/grant`).

**Публичные папки** (`/api/folder-shares` GET/POST + `/revoke`): `CreateFolderShare`/`ListMyFolderShares`/`RevokeFolderShare`. `FolderShareJson` собирает `url` через `ResolveOrigin` (см. выше), `kind=folder`. В «Мои публичные» вкладка `SharedPage` грузит и `/api/shares`, и `/api/folder-shares`, мержит (папки — `LinkCard` с иконкой папки; отзыв роутится по `kind`). Папку делают публичной из контекстного меню в «Файлах» (`dirMenu` → `createFolderShare`). **Публичная страница папки** `/f/:token` (`pages/PublicFolderPage.tsx`, вне `AppShell`, анонимна): `fetch GET /f/{token}/list?dir=` (`ResolveFolderShare` сервисным токеном) → грид подпапок + файлов; навигация по подпапкам, клик по фото/видео — встроенный вьювер (img/video по temp-URL), по файлу — скачивание. Корневой контейнер страницы — собственный scroll-viewer (`height: 100vh; overflow-y: auto`), потому что глобальный shell держит `body` с `overflow: hidden`; overlay встроенного viewer тоже прокручиваемый. Fallback в `Program.cs` пускает `/f/*` без авторизации (как `/v/*`).

**Публичные альбомы** (`/api/album-shares` GET/POST + `/revoke`, на `CloudApi`): `CreateAlbumShare`/`ListMyAlbumShares`/`RevokeAlbumShare`. `AlbumShareJson` собирает `url` через `ResolveOrigin` (см. выше), `kind=album`. В «Мои публичные» вкладка `SharedPage` грузит и `/api/album-shares` (иконка фото; отзыв по `kind`). Альбом делают публичным кнопкой «Поделиться» в `AlbumDetail` (`lib/share.createAlbumShare`). **Публичная страница альбома** `/al/:token` (`pages/PublicAlbumPage.tsx`, вне `AppShell`, анонимна): `fetch GET /al/{token}/list?cursor*` (`ResolveAlbumShare` сервисным токеном) → грид элементов с пагинацией «Показать ещё», встроенный вьювер. Корневой контейнер страницы — собственный scroll-viewer (`height: 100vh; overflow-y: auto`), чтобы большие альбомы прокручивались при глобальном `body overflow: hidden`; overlay встроенного viewer тоже прокручиваемый. Fallback в `Program.cs` пускает `/al/*` без авторизации.
