...
**Drag&drop загрузка** (`useFileDrop`): на `/photos`·`/videos`·`/files` перетаскивание файлов в окно подсвечивает зону (`.dropzone`/`.files-shell` + `.drop-overlay`) и запускает тот же `doUpload`. **Множественный выбор** (`useSelection` + `SelectionBar`): чекбоксы на карточках галерей (`.selbox`) и в таблице файлов (колонка `.selcell` + «выбрать все» в шапке); плавающая панель даёт групповые действия — удалить в корзину, копировать ссылки (галереи и файлы), добавить в альбом (только галереи, `useBulkMedia`). Hero-карточка последнего видео и папки в таблице из массового выбора исключены.
