...
- **Воспоминания** (`components/memories/MemoriesStrip.tsx`): горизонтальная лента «В этот день» на вкладке «Фото» (`PhotosPage`), грузит `/api/cloud/memories`, скрывается если пусто; клик по году → `Lightbox` со снимками группы. Стили `.memories`/`.mem-*` в `styles/shared.css`.
...
- **Общие компоненты** (`src/components/`): `Icon`, `MediaThumb` (srcset+sizes), `Lightbox` (оригинал через `/api/files/download`), `Modal`/`EmptyState`/`Loading`/`ContextMenu`/`RenameModal`/`ConfirmModal`/`PropertiesModal`/`SelectionBar` (плавающая панель групповых действий), альбомы (`AlbumCard`/`AlbumFormModal`/`PickMediaModal`/`AlbumDetail`).
...
**Поиск по имени**: поле в `Topbar` (Enter → `/files?q=`); `FilesPage` при наличии `q` грузит `GET /api/cloud/search?q=` (`CloudApi.SearchFiles`) и рендерит результаты вместо листинга папки (баннер «Результаты поиска» + «Очистить»; умные папки скрыты).
