...
/// Канал передачи квоты облака из контейнер-приложения в виджет хранилища.
/// Виджет живёт в отдельном процессе и не может ходить в gRPC — поэтому при
/// каждом обновлении квоты приложение кладёт три примитива в App Group
/// `UserDefaults`, а виджет читает их в своём `TimelineProvider`. Ключи
/// продублированы строками в `StorageSnapshot` виджета — это единственный
/// контракт между таргетами.
enum StorageWidgetBridge {
    ...
    private static var defaults: UserDefaults? {
        UserDefaults(suiteName: BarkCloudAppGroup.identifier)
    }
