...
- **Self-update веба** (`UpdateWebSelfAsync`): detached helper из образа web делает `pull web` → `up --force-recreate -d web` → `image prune -f` и сам исчезает (`--rm`). Работает под **Linux/WSL**: трюк BarkFluff «монтировать compose по тому же хостовому пути» требует Linux-пути (`/home/…`, `/mnt/…`). На **Windows Docker Desktop** хостовый путь — `C:\…`, невалидный как destination Linux-контейнера; здесь `RunWebHelperAsync` ловит это проверкой `IsWindowsPath(compose)` и возвращает понятную ошибку «обновите образ вручную (`docker compose pull web && up -d web`)», не запуская обречённый helper. Кнопка «Обновить веб-клиент» в UI активна — пригодность среды решает бэкенд по фактическому пути.
