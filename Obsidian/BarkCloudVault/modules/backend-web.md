...
- **Каркас** (`src/components/shell/`): `AppShell` (грузит `/api/me`, держит контексты `ShellContext`/`PageHeaderContext`), `Sidebar`/`Topbar`/`Footbar`. Страницы задают заголовок Topbar через `usePageHeader`. Browser title/favicon управляются через `DocumentHeadProvider`/`useDocumentHead`: базово показывается активная вкладка SPA и брендовый favicon `barkcloud-icon-brand-b.png`, при открытом файле/Lightbox — имя и закруглённое превью файла, при открытом альбоме — имя и закруглённая обложка; публичные `/v/*`, `/f/*`, `/al/*` задают title/favicon самостоятельно, так как идут вне `AppShell`.
