...
- `Endpoints/CloudApiEndpoints.cs` — группа `/api/*` для Фото/Видео/Файлов (см. раздел «Фото/Видео/Файлы») + **`GET /api/me`** (каркас: профиль/аватар/хранилище/версия/хост — собирает `PageDataBuilder.BuildShellAsync`). Также: `GET /api/cloud/path?entry=|dir=` (`GetPath` → `{ segments[{id,name}], fullPath }`, для «Показать в папке»), `POST /api/cloud/video/thumbnail` (`SetVideoThumbnail`, замена обложки видео), `POST /api/files/check-hash` (`CheckFileHash` → `{ fileId }`, клиентская дедупликация загрузки). **Воспоминания/Карта**: `GET /api/cloud/memories?month=&day=&perYear=` (`GetMemories` → `{ groups }`, маппер `CloudJson.MemoryGroup`) и `GET /api/cloud/map?limit=&cursorAt=&cursorId=` (`ListMediaLocations` → `{ points, nextCursor* }`, маппер `CloudJson.MapPoint`).
...
- **Роутинг** (`src/main.tsx`): `BrowserRouter` → layout-route `AppShell` + вложенные маршруты `photos`/`videos`/`files`/`map`/`favorites`/`trash`/`shared`/`settings` (`index` → `/photos`). Навигация в `Sidebar` через `NavLink` — без перезагрузки; меняется только `<Outlet/>`.
...
- **Карта** (`pages/MapPage.tsx`, маршрут `/map`, иконка `globe` в `Sidebar`): `react-leaflet` + OpenStreetMap-тайлы. Грузит точки постранично через `/api/cloud/map` (потолок 5000), клиентская grid-кластеризация по зуму (`Clusters`): кластеры — кружки со счётчиком (клик приближает), одиночные — маркеры с превью в попапе (клик → `Lightbox` по `id`). Зависимости `leaflet`/`react-leaflet` (+ `@types/leaflet`) в `package.json` — требуется `npm install`. Стили `.map-*`/`.cluster-label` в `styles/shared.css`.
