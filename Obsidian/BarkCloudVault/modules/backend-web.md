...
- **data-слой** (`src/lib/api.ts`): `api`/`apiGet`/`apiPost`/`pickFiles`/`uploadFile`, `ApiError`; 401 → `window.location='/login'`. `uploadFile` перед загрузкой считает SHA-256 (`crypto.subtle`) и спрашивает `/api/files/check-hash` — при совпадении переиспользует существующий `fileId` без перезаливки (поле `deduped`). Типы — `src/lib/types.ts`; форматирование — `src/lib/format.ts`; тема — `src/lib/theme.ts`.
...
- Файлы (FilesApi): `POST files/upload` — **прокси-загрузка**: `GetUploadUrl` → стрим байтов `HttpClient`-ом на **внутренний HTTP1-эндпоинт** Files `FilesService:Http1Base` (= хост из `FilesService:Host` + порт `FILES_HTTP1PORT`, по умолч. `7026`) `/upload/{fileId}` — минуя nginx/TLS и `ExternalEndpoint:Host`; публичный `upload.Url` — fallback. Возвращает `fileId`. `GET files/download?ids=` — временные ссылки на оригинал(ы) через `GetTempDownloadUrl`. `POST files/check-hash` (`CheckFileHash`) — клиентская дедупликация: `uploadFile` сначала шлёт SHA-256, при совпадении сервер отдаёт `fileId` и заливка пропускается.
