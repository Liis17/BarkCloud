...
- **BarkCloud.Drive.App** — UI на **WPF-UI** (`FluentWindow`) + **трей**. Три окна:
  ...
  - **`MainWindow`** (дашборд) — **аватар** (круглый, `Ellipse`+`ImageBrush`, байты через `GetAvatarAsync`) + имя
    ...
    Бар хранилища — **сегментированный по физическому диску сервера** (`EngineStatus.DiskTotal/DiskOther/DiskS3Bytes` из
    `GetUserStorageInfo`): тёмно-серый — не-S3 данные, коричневый `#9A4F1E` — S3 (облако), светло-серый — свободно; ниже легенда.
