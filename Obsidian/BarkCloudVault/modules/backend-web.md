...
- **Хуки** (`src/hooks/`): `useToast`, `useContextMenu`, `useInfiniteMedia` (cursor-пагинация + IntersectionObserver), `useAlbumMembership`, `useMediaActions`, `useShell`, `usePageHeader`, `useFileDrop` (drag&drop загрузка, depth-counter), `useSelection` (множественный выбор по id), `useBulkMedia` (групповые действия галерей: удалить/в альбом/копировать ссылки).
- **data-слой** (`src/lib/api.ts`): `api`/`apiGet`/`apiPost`/`pickFiles`/`uploadFile`, `ApiError`; 401 → `window.location='/login'`. `checkDuplicate` считает SHA-256 (`crypto.subtle`) и спрашивает `/api/files/check-hash` → `{exists, locations}`; при совпадении `useDuplicatePrompt` показывает модалку «такой файл уже есть» (имя + папка) с выбором для текущего файла или всех последующих дубликатов в этой загрузке: «загрузить ещё раз / пропустить / загрузить все / пропустить все». `uploadFile` всегда заливает новый блоб (серверный дедуп снят). Типы — `src/lib/types.ts`; форматирование — `src/lib/format.ts`; тема — `src/lib/theme.ts`.
