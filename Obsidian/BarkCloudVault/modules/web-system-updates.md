...
`/healthz` каждые 3 секунды. При наличии operation ID `/maintenance-status` подтверждает
завершение именно этой операции; после этого три ответа `healthz` возвращают браузер в
`/settings#system`. Если marker явно отсутствует (`204`), но timestamp процесса уже сменился,
страница использует тот же безопасный fallback — это позволяет восстановиться старым
установкам, где volume ещё не был подключён при первом self-update. Состояние `pending`
продолжает ожидание, сетевые/прочие ошибки не считаются успехом, а `failed` сразу показывает
сообщение и ссылку на защищённую диагностику. Для legacy-ссылки без operation ID используется
смена timestamp процесса.
Cache-Control `no-store`, непрозрачный operation ID и query-параметры не дают браузеру
застрять на свежей странице с тем же timestamp.
