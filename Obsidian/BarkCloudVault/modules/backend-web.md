...
- `Endpoints/CloudApiEndpoints.cs` — группа `/api/*` для Фото/Видео/Файлов (см. раздел «Фото/Видео/Файлы») + **`GET /api/me`** (каркас: профиль/аватар/хранилище/версия/хост — собирает `PageDataBuilder.BuildShellAsync`). Также: `GET /api/cloud/path?entry=|dir=` (`GetPath` → `{ segments[{id,name}], fullPath }`, для «Показать в папке»), `POST /api/cloud/video/thumbnail` (`SetVideoThumbnail`, замена обложки видео), `POST /api/files/check-hash` (`CheckFileHash` → `{ fileId }`, клиентская дедупликация загрузки).
