...
`/settings#system`. Для legacy-ссылки без operation ID используется смена timestamp процесса.
Состояние `failed` сразу показывает сообщение и ссылку на защищённую диагностику.
