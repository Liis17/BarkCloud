...
Маршрут SPA `/shared` (`pages/SharedPage.tsx`) — рабочая страница «Общий доступ» с единственной вкладкой «Мои публичные» (прежний demo-макет с фейковой статистикой/людьми/вкладками удалён). На mount грузит `GET /api/shares`, рендерит карточки ссылки (`LinkCard`: имя, URL, дата, счётчик переходов) с кнопками **«Скопировать»** (`navigator.clipboard`) и **«Отозвать»** (`window.confirm` → `POST /api/shares/revoke` → убрать из списка). Пустое состояние/`Loading`.
